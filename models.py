...
class Darknet(nn.Module):
    ...
    def forward(self, x=1,der=2, verbose=False):
        return self.forward_once(x,der, verbose=verbose)

    def forward_once(self, x,der, verbose=False):
        ...
        for i, module in enumerate(self.module_list):
            ...
            # print(x.size(),i)
            if i == 112:
                # print(x.size(),der.size())
                x_masked = x * der  # 将掩膜应用于特征图，得到一个新的变量 x_masked
                output = x_masked + x  # 将掩膜应用后的特征图与原特征图相加得到最终输出
                x = output
