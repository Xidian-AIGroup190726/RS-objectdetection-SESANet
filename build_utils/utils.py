...
def gradient_blur_bchw(input_bchw):
    ####计算卷积核的数字
    z1 = z2 = z3 = torch.zeros((5,5))
    m = 0
    for i,r in enumerate(range(-2,3)):
        for j,c in enumerate(range(-2,3)):
            m = m + r**2
    for i,r in enumerate(range(-2,3)):
        for j,c in enumerate(range(-2,3)):
            z1[i,j] = r/m
    ...
    for i,r in enumerate(range(-2,3)):
        for j,c in enumerate(range(-2,3)):
            z3[i,j] = r*(c*c-2)/m	
    z1 = z1.unsqueeze(0).repeat(3,1,1).unsqueeze(0).cuda(1)
    z2 = z2.unsqueeze(0).repeat(3,1,1).unsqueeze(0).cuda(1)
    z3 = z3.unsqueeze(0).repeat(3,1,1).unsqueeze(0).cuda(1)
    grad_z1_bchw = F.conv2d(input_bchw, weight=z1, stride=1,padding = 2)
    grad_z2_bchw = F.conv2d(input_bchw, weight=z2, stride=1,padding = 2)
    grad_z3_bchw = F.conv2d(input_bchw, weight=z3, stride=1,padding = 2)
    k = grad_z1_bchw - 3.4*grad_z2_bchw -2*grad_z3_bchw
    v1 = v2 = v3 = torch.zeros((5,5))
    m = 0
    for i,r in enumerate(range(-2,3)):
        for j,c in enumerate(range(-2,3)):
            m = m + c**2
    ...
    for i,r in enumerate(range(-2,3)):
        for j,c in enumerate(range(-2,3)):
            v3[i,j] = (c**3-3.4*c)/m	
    v1 = v1.unsqueeze(0).repeat(3,1,1).unsqueeze(0).cuda(1)
    v2 = v2.unsqueeze(0).repeat(3,1,1).unsqueeze(0).cuda(1)
    v3 = v3.unsqueeze(0).repeat(3,1,1).unsqueeze(0).cuda(1)
    grad_v1_bchw = F.conv2d(input_bchw, weight=v1, stride=1,padding = 2)
    grad_v2_bchw = F.conv2d(input_bchw, weight=v2, stride=1,padding = 2)
    grad_v3_bchw = F.conv2d(input_bchw, weight=v3, stride=1,padding = 2)
    v = grad_v1_bchw - 2*grad_v2_bchw -3.4*grad_v3_bchw	
    k = torch.abs(k) +torch.abs(v)
    return k
...
#导数计算部分
def der_need(b,gi,gj,a,imgs,anchors):
    #获得xy，xy
    result_list=[]
    for i,j in enumerate(b):
        imgs_size = np.int(imgs.size(2)/8)
        # print(imgs_size,'aaa')
        x1 ,x2 = gi[i] - anchors[a[i],0]/2 , gi[i] + anchors[a[i],0]/2 
        y1 ,y2 = gj[i] - anchors[a[i],1]/2 , gj[i] + anchors[a[i],1]/2
        input_bchw = imgs[j]
        input_bchw =input_bchw.cuda(1)
        # print(a[i])
        # print(anchors[a[i],0],anchors[a[i],1])
        input_bchw = input_bchw.unsqueeze(0)
        der = torch.abs(gradient_blur_bchw(input_bchw.float()))
        der = max_min(der)
        der[der<0.01]=0
        der = cv2.morphologyEx(der[0].squeeze().cpu().numpy().astype(np.uint8),cv2.MORPH_OPEN,kernel=np.ones((5,5)))
        der = torch.tensor(der).unsqueeze(0)
        der = der.permute(1,2,0)#whc
        # print(der.shape)
        fin_der = cv2.resize(np.array(der),(imgs_size,imgs_size))
        # print(der.shape)
        # print(der.shape)
        if x2>51:
            x2 = 51
        ...
        if int(x1) == int(x2) or int(y1)==int(y2):
            # print('error')
            soc_der = torch.tensor(1).cuda(1)
        if int(x1) > int(x2) or int(y1) > int(y2):
            soc_der = torch.tensor(1).cuda(1)
        else:
            # der = np.array(der)
            # print(der.shape)
            soc_der = fin_der[int(x1):int(x2),int(y1):int(y2)]
            # print(der.size())
            soc_der = der.float().mean()
            soc_der = torch.tensor(soc_der).cuda(1)
            # print(der)
        # print(soc_der)
        result_list.append(soc_der)
    return result_list
...
def compute_loss(p, targets, model,imgs):  # predictions, targets, model
    device = p[0].device
    lcls0 = torch.zeros(1, device=device)
    ...
    tcls, tbox, indices, anchors, der = build_targets(p, targets, model,imgs)  # targets
    der = torch.tensor(der)
    # der = der.to(device)
    # print(der)
    ...
    # Define criteria
    Der_BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['cls_pw']], device=device), reduction=red)
    ...
    if g > 0:
        Der_BCEcls = FocalLoss(Der_BCEcls, g)
        ...
        # #这里对第三层的giou做改变
            ...
            if model.nc > 1:  # cls loss (only if multiple classes)
                ...
                if i == 2:
                    ...
                    t[range(nb), tcls[i]] = cp
                    for k in range(len(der)):
                        # lcls1 += BCEcls(ps[k:k+1, 5:], t[k:k+1,:])
                        lcls1 += Der_BCEcls(ps[k:k+1, 5:], t[k:k+1,:])*(der[k]+1)
                    lcls0 +=(lcls1)/len(der)

                # else:
                #     t = torch.full_like(ps[:, 5:], cn, device=device)  # targets
                #     t[range(nb), tcls[i]] = cp
                #     lcls0 += Der_BCEcls(ps[:, 5:], t)  # BCE
                #else:
                    #t = torch.full_like(ps[:, 5:], cn, device=device)  # targets
                    #t[range(nb), tcls[i]] = cp
                    #lcls += BCEcls(ps[:, 5:], t)  # BCE
...
#正样本匹配机制
def build_targets(p, targets, model,imgs):
    ...
    for i, j in enumerate(model.yolo_layers):  # j: [89, 101, 113]
        ...
        if i ==2:
            der = der_need(b,gi,gj,a,imgs,anchors)
        # der = 0
        # # Append
    ...
    # print(tcls[0].size())
    return tcls, tbox, indices, anch, der
