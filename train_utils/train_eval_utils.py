...
def gradient_blur_bchw(input_bchw,device):
    ####计算卷积核的数字
    ...
    for i,r in enumerate(range(-2,3)):
        for j,c in enumerate(range(-2,3)):
            z3[i,j] = r*(c*c-2)/m	
    z1 = z1.unsqueeze(0).repeat(3,1,1).unsqueeze(0).to(device)
    z2 = z2.unsqueeze(0).repeat(3,1,1).unsqueeze(0).to(device)
    z3 = z3.unsqueeze(0).repeat(3,1,1).unsqueeze(0).to(device)
...
def train_one_epoch(model, optimizer, data_loader, device, epoch,
    ...
    for i, (imgs, targets, paths, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if multi_scale:
            ...
            size = np.int( img_size/8)
            img_need = imgs
            
        
        der_ori = gradient_blur_bchw(imgs,device).cpu().numpy()
        # print(der_ori.shape)
        # plt.imshow(np.transpose(der_ori[0],(1,2,0)))
        # plt.show()
        resized_images = []
        # print(der_ori.shape,"der")
        for i in range(imgs.shape[0]):
            # print(i)
            image = np.transpose(der_ori[i], (1, 2, 0)) # 获取第 i 张图像
            resized_image = cv2.morphologyEx(image,cv2.MORPH_CLOSE,kernel=np.ones((5,5)))
            # print(der.shape,123123)
            resized_image = cv2.morphologyEx(resized_image,cv2.MORPH_OPEN,kernel=np.ones((5,5)))
            # print(size,'img_size')
            resized_image = cv2.resize(resized_image, (size,size))  # 进行放缩
            resized_image = np.expand_dims(resized_image, axis=0)  # 将通道维度移回正确的位置
            # der = np.transpose(resized_image,(1,2,0))
            resized_image = max_min(resized_image)
            resized_image[resized_image<0.95] = torch.tensor(0.0)
            resized_image[resized_image>0] = torch.tensor(1.0)
            # plt.imshow(der)
            # plt.show()
            
            
            resized_images.append(resized_image)
        resized_images = np.stack(resized_images)  # 将放缩后的图像重新堆叠为批次
        # print(resized_image.shape)

        der = torch.tensor(resized_images).to(device)
        
        
        
        # 混合精度训练上下文管理器，如果在CPU环境中不起任何作用
        with amp.autocast(enabled=scaler is not None):
            pred = model(x = imgs,der = der)

            # loss
            loss_dict = compute_loss(pred, targets, model,img_need)
...
def evaluate(model, data_loader, coco=None, device=None):
    ...
    for imgs, targets, paths, shapes, img_index in metric_logger.log_every(data_loader, 100, header):
        ...
        # plt.show()
        resized_images = []
        w = np.int(imgs.shape[2]/8)
        h = np.int(imgs.shape[3]/8)
        # print(der_ori.shape,"der")
        for i in range(imgs.shape[0]):
            # print(i)
            image = np.transpose(der_ori[i], (1, 2, 0)) # 获取第 i 张图像
            ...
            # print(der.shape,123123)
            resized_image = cv2.morphologyEx(resized_image,cv2.MORPH_OPEN,kernel=np.ones((5,5)))
            resized_image = cv2.resize(image, (h, w))  # 进行放缩
            resized_image = np.expand_dims(resized_image, axis=0)  # 将通道维度移回正确的位置
            # der = np.transpose(resized_image,(1,2,0))
            resized_image = max_min(resized_image)
            resized_image[resized_image<0.95] = torch.tensor(0.0)
            ...
            resized_image = np.repeat(resized_image,256,axis = 0)
            # plt.imshow(der)
            # plt.show()
        ...
        pred = model(x=imgs,der = der)[0]  # only get inference result
